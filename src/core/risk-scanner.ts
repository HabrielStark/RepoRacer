...
export function scanRisks(diff: DiffSummary, config: RepoRacerConfig, fullPatch = diff.patchPreview): RiskFlag[] {
  ...
  if (containsSecretPattern(fullPatch)) {
    flags.push({
      ...
      code: "SECRET_PATTERN",
      message:
        "Patch contains a string that looks like a secret; previews are redacted, but the patch should be reviewed."
    });
  }
...
export function hasCriticalRisk(flags: readonly RiskFlag[]): boolean {
...
function ciLooksWeakened(patch: string): boolean {
  const normalized = patch.toLowerCase();
  return (
    normalized.includes("continue-on-error: true") ||
...
function testLooksWeakened(patch: string): boolean {
  const normalized = patch.toLowerCase();
  return (
...
function containsSecretPattern(patchPreview: string): boolean {
  return (
    /\[REDACTED_(?:API_KEY|TOKEN|PRIVATE_KEY|GITHUB_TOKEN|AWS_ACCESS_KEY|CREDENTIALS)\]/.test(patchPreview) ||
    /\b(?:sk-[A-Za-z0-9_-]{20,}|sk-ant-[A-Za-z0-9_-]{20,}|gh[pousr]_[A-Za-z0-9_]{20,}|AKIA[0-9A-Z]{16})\b/.test(
      patchPreview
    ) ||
    /^[+\-\s]*[A-Za-z_][A-Za-z0-9_]*(?:TOKEN|SECRET|PASSWORD|API_KEY|ACCESS_KEY)[A-Za-z0-9_]*\s*=/im.test(
      patchPreview
    ) ||
