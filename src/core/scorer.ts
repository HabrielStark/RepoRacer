...
export function scoreResult(input: ScoreInput): ScoreBreakdown {
  ...
  const minimality = minimalityScore(
    input.task.changedFiles.length,
    input.diff.changedFiles.length,
    input.task.changedLines,
    input.diff.changedLines
  );
...
export function testScore(result: TestRunResult): number {
  if (result.skipped) {
    return 40;
  }
...
export function changedFilesOverlapScore(expectedFiles: readonly string[], actualFiles: readonly string[]): number {
...
export function patchSimilarityScore(humanPatch: string, agentPatch: string): number {
...
export function diffSizeScore(expectedLines: number, actualLines: number): number {
  if (actualLines === 0) {
    return 0;
  }
  if (expectedLines === 0) {
...
export function minimalityScore(
...
export function speedScore(durationMs: number, timeoutMs: number): number {
  if (timeoutMs <= 0) {
    return 50;
  }
...
export function riskPenaltyScore(risks: readonly RiskFlag[]): number {
  return Math.min(
    100,
...
function normalizedPatchLines(patch: string): Set<string> {
  return new Set(
    patch
      .split(/\r?\n/)
      .map((line) => line.trim())
