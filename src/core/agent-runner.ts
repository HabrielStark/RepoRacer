...
export async function runAgentTask(options: RunAgentTaskOptions): Promise<RepoRacerResult> {
  ...
  const baselineLogPath = runPath(
    options.repoRoot,
    options.runId,
    "logs",
    `${options.task.id}-${safeAgent}.baseline.log`
  );
  ...
  const commandResult =
    install !== null && install.exitCode !== 0
  ...
  const hiddenTestPatchApplied =
    options.config.evaluationMode === "hidden-target-tests" && hiddenTestPatchPath !== null;
  let hiddenPatchError: string | null = null;
    ...
    hiddenPatchError !== null
      ...
      : commandResult.exitCode === 0 && !commandResult.timedOut
        ? await runTests(
            ...
            timeoutMs,
            testLogPath,
            hiddenTestPatchApplied ? "hidden" : "agent",
            options.config.sandbox
          )
        : skippedTestsAfterAgentFailure(options.config.testCommand);
  const hiddenTests =
    hiddenTestPatchApplied && commandResult.exitCode === 0 && !commandResult.timedOut
      ? {
          ...tests,
...
async function runInstallIfConfigured(
  _repoRoot: string,
  worktreePath: string,
  config: RepoRacerConfig,
  timeoutMs: number
...
async function discardInstallSideEffects(worktreePath: string): Promise<string[]> {
...
async function runAgentCommand(
  ...
  if (options.agent.command === null || options.agent.command.trim().length === 0) {
    return syntheticResult(
      ...
      worktreePath,
      127,
      `Agent command is not configured: ${options.agent.name}. Add a command in .reporacer/config.json or install/configure the matching CLI and run reporacer doctor.`,
      false
    );
  }
...
async function applyHumanPatch(
  worktreePath: string,
  patchPath: string,
...
function failedHiddenPatchTests(command: string | null, error: string): TestRunResult {
  return {
    skipped: false,
    command,
    passed: false,
...
function skippedTestsAfterAgentFailure(command: string | null): TestRunResult {
  return {
    skipped: true,
    command,
    passed: false,
...
function testResultLog(result: TestRunResult, label: string): string {
  return [
    `# ${label}`,
