...
export function renderShareMarkdown(summary: RepoRacerSummary): string {
  const winner = redactSecrets(summary.winner ?? "none");
  const rows = summary.leaderboard
    .map(
      (row) =>
        `| ${escapePipe(redactSecrets(row.agent))} | ${row.solved}/${row.total} | ${row.averageScore} | ${Math.round(row.testsPassedRate * 100)}% | ${row.riskFlags} |`
    )
    .join("\n");
...
export function renderBadgeSvg(summary: RepoRacerSummary): string {
...
export async function writeShareArtifacts(
  repoRoot: string,
  summary: RepoRacerSummary,
  options: { markdown: boolean; badge: boolean } = { markdown: true, badge: true }
): Promise<{ markdownPath: string | null; badgePath: string | null }> {
