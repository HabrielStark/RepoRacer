...
import { AgentRuntime, resolveAgents } from "../agents/types.js";
...
import {
  buildLeaderboard,
  resetResultFiles,
  winnerFromLeaderboard,
  writeResult,
  writeSummary
} from "../core/results.js";
import { mineTasks } from "../core/task-miner.js";
import { RepoRacerConfig, RepoRacerResult, RepoRacerSummary, RunRepoRacerOptions } from "../schemas/types.js";
...
export async function runRepoRacer(options: RunRepoRacerOptions = {}): Promise<RepoRacerSummary> {
  const repoRoot = await getRepoRoot(options.repoRoot ?? process.cwd());
  const config = await loadConfig(repoRoot);
  const reportConfig =
    options.redactReport === undefined
      ? config.report
      : {
          ...config.report,
  ...
  if (tasks.length === 0) {
    throw new Error(
      "No suitable historical tasks found. Try lowering commit filters or using a repository with more commits."
    );
  }
  await emitPluginHook(options, "onTasksMined", { repoRoot, runId, tasks });

  const results: RepoRacerResult[] = [];
  ...
  await runLimited(runItems, limit, async ({ task, agent }) => {
    ...
    const result = await runAgentSafely(
      ...
      task,
      agent,
      options.baselineCheck ?? effectiveConfig.baselineCheck
    );
    completedItems += 1;
    emitProgress(options, {
      event: "agent_finish",
  ...
  const summary: RepoRacerSummary = {
    ...
    results: results.sort(
      (left, right) => left.taskId.localeCompare(right.taskId) || left.agentName.localeCompare(right.agentName)
    )
  };
  ...
  const publicReportPath =
    effectiveConfig.report.audience === "public"
  ...
  await fs.writeFile(
    repoRacerPath(repoRoot, "last-run.txt"),
...
async function emitPluginHook<T extends keyof NonNullable<RunRepoRacerOptions["plugins"]>[number]>(
  options: RunRepoRacerOptions,
  hookName: T,
  event: Parameters<NonNullable<NonNullable<RunRepoRacerOptions["plugins"]>[number][T]>>[0]
): Promise<void> {
...
interface ProgressEvent {
  event: "agent_start" | "agent_finish";
  completed: number;
  total: number;
  taskId: string;
  agentName: string;
...
function emitProgress(options: RunRepoRacerOptions, event: ProgressEvent): void {
  if (!options.progress) {
    return;
  }
...
async function runAgentSafely(
  ...
  task: Parameters<typeof runAgentTask>[0]["task"],
  agent: AgentRuntime,
  baselineCheck: boolean
): Promise<RepoRacerResult> {
  try {
    ...
    await writeGeneratedFile(
      repoRoot,
      logPath,
      [`# internal error`, `task: ${task.id}`, `agent: ${agent.name}`, "", message].join("\n")
    );
    return {
...
export function renderRunSummary(summary: RepoRacerSummary): string {
...
async function prepareRunDirectories(repoRoot: string, runId: string): Promise<void> {
...
async function writeConfigSnapshot(repoRoot: string, runId: string, config: unknown): Promise<void> {
...
async function runLimited<T>(items: readonly T[], limit: number, worker: (item: T) => Promise<void>): Promise<void> {
