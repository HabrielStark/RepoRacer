...
export async function writeResult(repoRoot, runId, result) {
...
export async function resetResultFiles(repoRoot, runId) {
    const paths = [
...
export function buildLeaderboard(results) {
    ...
    return [...groups.entries()]
        ...
        const testsPassed = testsWithCommand.filter((result) => result.tests.passed &&
            (result.hiddenTests === null || result.hiddenTests.skipped || result.hiddenTests.passed)).length;
        ...
        .sort((left, right) => right.solved - left.solved || right.averageScore - left.averageScore || left.riskFlags - right.riskFlags);
}
export function winnerFromLeaderboard(leaderboard) {
    return leaderboard[0]?.agent ?? null;
}
export async function writeSummary(repoRoot, runId, summary) {
