...
export async function runCommand(command, options) {
    ...
    const renderedCommand = options.sandbox?.mode === "docker" && dockerScript !== null
...
export function buildDockerCommand(command, cwd, sandbox) {
...
export function redactSecrets(input) {
    ...
    output = output.replace(/\bhttps?:\/\/([^:\s/@]+):([^@\s/]+)@/gi, (match) => match.replace(/\/\/[^:\s/@]+:[^@\s/]+@/, "//[REDACTED_CREDENTIALS]@"));
    output = output.replace(/\b(Bearer\s+)[A-Za-z0-9._~+/-]{16,}={0,2}\b/gi, "$1[REDACTED_TOKEN]");
    output = output.replace(/^([A-Za-z_][A-Za-z0-9_]*(?:TOKEN|SECRET|PASSWORD|API_KEY|ACCESS_KEY)[A-Za-z0-9_]*\s*=\s*).+$/gim, "$1[REDACTED]");
    output = output.replace(/\b(api[_-]?key|token|secret|password|authorization)\b(\s*[:=]\s*)["']?[^"'\s,;]{8,}["']?/gi, "$1$2[REDACTED]");
    return output;
}
...
export function commandResultLog(result, label) {
    return [
        `# ${label}`,
        `timestamp: ${nowIso()}`,
        `cwd: ${result.cwd}`,
...
//# sourceMappingURL=process-safe.js.map