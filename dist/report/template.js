...
export function renderReportHtml(summary, options = {
    includeLogs: true,
...
function renderLeaderboard(summary, options) {
...
function renderResults(summary, options) {
...
function renderRisks(risks) {
    ...
    return risks
        .map((risk) => `<div class="risk"><strong>${escapeHtml(risk.level)}</strong> ${escapeHtml(risk.code)}<br /><span class="muted">${escapeHtml(risk.message)}${risk.file === undefined ? "" : `: ${escapeHtml(risk.file)}`}</span></div>`)
        .join("");
}
